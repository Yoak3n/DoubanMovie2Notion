    ...
    <div class="config-wrapper">
        ...
        :show-require-mark="false">
            <n-form-item label="DatabaseID" path="databaseid">
                <n-input v-model:value="configuration.database_id"/>
            </n-form-item>
...
const  configuration =ref<Configuration>({database_id:"",token:""})
...
const setConfiguration = ()=>{
    ...
    WriteConfig(name,configuration.value.database_id,configuration.value.token).then((msg)=>{
        if(msg!="success"){
            $notification.error({
                content:"出现错误请重试："+msg,
                duration:1000,
